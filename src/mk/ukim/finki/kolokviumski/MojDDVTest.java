...
class Fiskalna {
    ...
    public String toString() {
        return String.format("%10s\t%10d\t %9.5f", id, totalAmount, returnedDDV());
    }
}
...
class MojDDV {
    ...
    void printStatistics(OutputStream outputStream) {
        PrintWriter pw = new PrintWriter(outputStream);
        DoubleSummaryStatistics dbs = fiskalni.stream().mapToDouble(f -> f.returnedDDV()).summaryStatistics();
        pw.printf("min:\t%.2f\nmax:\t%.2f\nsum:\t%.2f\ncount:\t%d\navg:\t%.2f\n", dbs.getMin(), dbs.getMax(), dbs.getSum(), dbs.getCount(), dbs.getAverage());
        pw.flush();
    }
