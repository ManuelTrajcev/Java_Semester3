...
interface LogProcessor<IN> {
    ArrayList<IN> processLogs(ArrayList<IN> logs);
}

class LogSystem<IN extends ILog> {
    //TODO add instance variable(s)

    //TODO constructor
    ArrayList<IN> logsList;

    public LogSystem(ArrayList<IN> logsList) {
    ...
    void printResults() {
        ...
        LogProcessor<IN> firstLogProcessor = logsList -> logsList.stream().filter(l -> l.getType().equals("INFO")).collect(Collectors.toCollection(() -> new ArrayList<>()));

        LogProcessor<IN> secondLogProcessor = logsList -> logsList.stream().filter(l -> l.getMessage().length() < 100).collect(Collectors.toCollection(() -> new ArrayList<>()));

        LogProcessor<IN> thirdLogProcessor = logs -> logsList.stream().sorted().collect(Collectors.toCollection(() -> new ArrayList<>()));
