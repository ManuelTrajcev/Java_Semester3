...
class Scheduler<T> {
    ...
    public boolean remove(Timestamp<Integer> t) {
    ...
    public Timestamp<T> next() {
        List<Timestamp<T>> filtered = schedule.stream().filter(i -> i.getTime().isAfter(LocalDateTime.now())).collect(Collectors.toList());
        return filtered.get(0);
    }

    public Timestamp<T> last() {
        List<Timestamp<T>> filtered = schedule.stream().filter(i -> i.getTime().isBefore(LocalDateTime.now())).collect(Collectors.toList());
        return filtered.get(filtered.size()-1);
    }

    public List<Timestamp<T>> getAll(LocalDateTime begin, LocalDateTime end) {
        return schedule.stream().filter(i -> i.getTime().isAfter(begin) && i.getTime().isBefore(end)).collect(Collectors.toList());
    }
}
