...
public class ArrayMeanWordLength {
    ...
    public static String wordClosestToAverageLength(Array<String> arr) {
        double avg = 0;
        String closest = "";
        double closestLength = 100;
        boolean isFirst = true;
        for (int i = 0; i < arr.getSize(); i++) {
            avg += arr.get(i).length();
        }
        avg /= arr.getSize();
        ...
        for (int i = 0; i < arr.getSize(); i++) {
            if (isFirst) {
                closest = arr.get(i);
                closestLength = Math.abs(arr.get(i).length() - avg);
                isFirst = false;
                continue;
            }
            if (Math.abs(arr.get(i).length() - avg) < closestLength) {
                closest = arr.get(i);
                closestLength = Math.abs(arr.get(i).length() - avg);
            } else if (Math.abs(arr.get(i).length() - avg) == closestLength) {
                if (closest.length() < arr.get(i).length()) {
                    closest = arr.get(i);
                }
            }
