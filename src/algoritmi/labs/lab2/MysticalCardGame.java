...
public class MysticalCardGame {
    ...
    public static int getScore(Card card) {
        return card.getHealth() * card.getMagicPower();
    }

    public static void startDuel(SLL<Card> firstSorcererCards, SLL<Card> secondSorcererCards) {
        SLLNode<Card> strongestFirst = firstSorcererCards.getFirst();
        SLLNode<Card> currFirst = firstSorcererCards.getFirst();
        SLLNode<Card> strongestSecond = secondSorcererCards.getFirst();
        SLLNode<Card> currSecond = secondSorcererCards.getFirst();
        for (int i = 1; i < 8; i++) {
            currFirst = currFirst.succ;
            currSecond = currSecond.succ;
            if (getScore(currFirst.element) > getScore(strongestFirst.element)) {
                strongestFirst = currFirst;
            }
            if (getScore(currSecond.element) > getScore(strongestSecond.element)) {
                strongestSecond = currSecond;
            }
        }

        firstSorcererCards.delete(strongestFirst);

        currSecond = secondSorcererCards.getFirst();
        for (int i = 0; i < 3; i++) {
            currSecond = currSecond.succ;
        }

        secondSorcererCards.insertAfter(strongestFirst.element, currSecond);
